...
EVALUATION_INTERVAL = 40  # Evaluate every 1000 steps

PERFORMANCE_THRESHOLD = 6# Threshold for performance to resume training
experience_dict = {}
reward_history = []
score = 0
node_dir = "/home/bidhya/workspace/STRL-singlemodel/rl/model"
...
def main(node_name):
    ...
    while True:
        try:
            ...
            print("state dict", states_dict)
    
            result = '/'.join(str(value) for value in states_dict.values()).split(" ")[0]
            prefix_name_with_packet = f"{states_dict['name']}/{node_name}/{states_dict['type']}"      
            ...
            print("Analysis Counter: ", counter, " Suppression time", st, " Reward", reward)
            counter = counter + 1
