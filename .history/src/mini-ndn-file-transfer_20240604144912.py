...
def receiveFile(node, prefix, filename):
    info ("Fetching file: {} \n".format(filename))
    cmd = 'ndncatchunks -v -p cubic --log-cwnd log-cwnd --log-rtt log-rtt {}/{} > {} 2> catchunks-{}.log &'.format(prefix, filename, filename, filename)
    node.cmd(cmd)
...
if __name__ == '__main__':
    ...
    base_path = "/home/bidhya/Desktop/STRL_separate_suppression_time/STRL/files/"
    file_list = ["transfer_.dat",
                 "transfer.dat",
                 "transfer1.dat"]
    ...
    for file in file_list:
        ...
        for c in consumers:
            receiveFile(c, producers_prefix[producer_name], producer_name + ".txt")
