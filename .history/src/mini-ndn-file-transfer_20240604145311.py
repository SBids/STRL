...
def receiveFile(node, prefix, filename, full_filename):
    ...
    cmd = 'ndncatchunks -v -p cubic --log-cwnd log-cwnd --log-rtt log-rtt {}/{} > {} 2> catchunks-{}-{}.log &'.format(prefix, filename, full_filename, full_filename, filename)
    print(f"Received File: {full_filename}")
    node.cmd(cmd)
