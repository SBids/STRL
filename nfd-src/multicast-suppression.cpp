...
NameTree::getSuppressionTimer(const std::string& name)
{
  ...
  // time::milliseconds suppressionTimer (getRandomNumber(static_cast<int> (2*suppressionTime))); // timer is randomized value
   time::milliseconds suppressionTimer (static_cast<int> (suppressionTime)); //without suppression mechanism
...
*/
void
EMAMeasurements::addUpdateEMA(int duplicateCount, bool wasForwarded, std::string name, std::string segment)
{
  ...
  updateDelayTime(wasForwarded, name, segment);
...
void
EMAMeasurements::updateDelayTime(bool wasForwarded, std::string name, std::string segment)
{
  ...
  */
  ...
  message.put("ewma_dc", round(m_expMovingAverageCurrent*100)/100);
  message.put("seg_name", segment);
  ...
  auto suppression_time = m_fifo.fifo_handler(messageString);
  ...
  //   }
    ...
    m_currentSuppressionTime =  std::min(std::max(m_minSuppressionTime, suppression_time), maxSuppressionTime);
...
MulticastSuppression::recordData(Data data, bool isForwarded)
{
    ...
    if (itr_timer != m_objectExpirationTimer.end())
    {
      ...
      if (m_interestHistory.count(name) > 0)
      {
        ...
        NFD_LOG_INFO("Duplicate count after data satisfied " << duplicateCountInterest);
...
MulticastSuppression::setUpdateExpiration(time::milliseconds entryLifetime, Name name, char type)
{
  ...
  auto eventId = getScheduler().schedule(entryLifetime, [=]  {
    if (vec->count(name) > 0)
    {
      ...
      auto duplicateCountInterest = getDuplicateCount(name, type);

      updateMeasurement(name,  type);
      if(type=='i'){
        NDN_LOG_INFO("Duplicate Count: " << duplicateCountInterest << " Analysis History Interest: " << name);
      }
      else{
        NDN_LOG_INFO("Duplicate Count: " << duplicateCountInterest << " Analysis History Data: " << name);
      }
      // NFD_LOG_INFO("Trying to get duplicate count before deleting interest or data" << duplicateCountInterest << " Name " << name);
      vec->erase(name);
...
MulticastSuppression::updateMeasurement(Name name, char type)
{
    ...
    if (it == vec->end())
    {
      ...
      emaEntry->addUpdateEMA(duplicateCount, wasForwarded, name.toUri(), segment);
    ...
    else
    {
      ...
      it->second->addUpdateEMA(duplicateCount, wasForwarded, name.toUri(), segment);
