...
protected: // upper interface to be invoked in subclass (receive path termination)
  ...
  // void
  // scheduleEntry(Name name, scheduler::EventId& eid, TimeMilliseconds)
  // {
  //   m_scheduledEntry.emplace(name, eid);
  // }

  void scheduleEntry(Name name, scheduler::EventId& eid, time::milliseconds waitTime)
  {
    m_scheduledEntry.emplace(name, std::make_pair(eid, waitTime));
  }
...
private:
  ...
  // Define the map with std::pair as the value type
  std::map<ndn::Name, std::pair<const scheduler::EventId, time::milliseconds>> m_scheduledEntry;
};
