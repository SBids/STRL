...
LinkService::sendInterest(const Interest& interest)
{
  ...
  m_scheduledEntry.emplace(entry_name, std::make_pair(eventId, suppressionTime));
...
LinkService::sendData(const Data& data)
{
  ...
  auto eventId = getScheduler().schedule(suppressionTime, [this, data, entry_name, suppressionTime] {
    ...
    ++this->nOutData;
    doSendData(data);
    NFD_LOG_INFO("Wait Time for " << suppressionTime << " Analysis History Data sent finally:, via multicast face " << data.getName());
    // NFD_LOG_INFO("Average packet count data outgoing " << nOutData);
...
LinkService::cancelIfSchdeuled(Name name, int type)
{
  ...
  if (it != m_scheduledEntry.end()) {
    ...
    NFD_LOG_INFO("Deleted record for " << name << " suppression time " << it->second.second);
    m_scheduledEntry.erase(entry_name);
...
LinkService::receiveInterest(const Interest& interest, const EndpointId& endpoint)
{
  ...
  if (this->getFace()->getLinkType() == ndn::nfd::LINK_TYPE_MULTI_ACCESS)
  {
    ...
    auto entry_name = name.appendNumber(0);
    auto it = m_scheduledEntry.find(entry_name);
    if (it != m_scheduledEntry.end()) {
        auto waitTime = it->second.second;
        it->second.first.cancel();
        m_scheduledEntry.erase(entry_name);
        NDN_LOG_INFO("Interest drop by suppression, with name " << name << " overheard, duplicate forwarding dropped");
        m_multicastSuppression.recordInterest(interest, false, waitTime);
      }   
    // if (cancelIfSchdeuled(interest.getName(), 0)) // checks whether the packet is in measurement table and drops if its already there
    m_multicastSuppression.recordInterest(interest, false, time::milliseconds(0));
  }
  ++this->nInInterests;
...
LinkService::receiveData(const Data& data, const EndpointId& endpoint)
{
  ...
  if (this->getFace()->getLinkType() == ndn::nfd::LINK_TYPE_MULTI_ACCESS)
  {
    ...
    auto entry_name = name.appendNumber(1);
    auto it = m_scheduledEntry.find(entry_name);
    if (it != m_scheduledEntry.end()) {
        auto waitTime = it->second.second;
        it->second.first.cancel();
        m_scheduledEntry.erase(entry_name);
        NDN_LOG_INFO("Data drop by suppression, with name " << name << " overheard, duplicate forwarding dropped");
        m_multicastSuppression.recordData(data, false, waitTime);
      }   
    ...
    m_multicastSuppression.recordData(data, false, time::milliseconds(0));
  }
